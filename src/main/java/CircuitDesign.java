...
public class CircuitDesign {
    ...
    private CircuitDesign(InputReader reader, OutputWriter writer) {
        this.reader = reader;
        this.writer = writer;
    }
    ...
    class TwoSAT {
        ...
        void setup() {
            ...
            for (int i = 0; i < 2 * numVars; i++) {
                if (!visited[i]) {
                    visited[i] = true;
                    postOrder(adjR, visited, i, order);
                    order.add(i);
                }
            }
        ...
        void postOrder(ArrayList<Integer>[] adj, boolean[] visited, int x, LinkedList<Integer> order) {
            for (int nextX : adj[x]) {
                if (!visited[nextX]) {
                    visited[nextX] = true;
                    postOrder(adj, visited, nextX, order);
                    order.add(nextX);
                }
            }
        }

        void exploreScc(ArrayList<Integer>[] adj, boolean[] visited, int x, HashSet<Integer> scc) {
            scc.add(x);
            for (int nextX : adj[x]) {
                if (!visited[nextX]) {
                    visited[nextX] = true;
                    exploreScc(adj, visited, nextX, scc);
                }
            }
        }
    }
